namespace :db do
  ...
  task restore_planning: :environment do |task|
    ...
    cmd = "PGPASSWORD='#{pass}' pg_restore -a -h '#{host}' -U '#{user}' -d '#{database}' -vxOW -j 8 -t places -t counties -t neighborhoods_poly -t rpas -t tazs -t  tod_service_area_poly #{Rails.root}/lib/import/fbrbuilds_dev.dump"
    puts cmd
    exec cmd
    end
  end
